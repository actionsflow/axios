...
export const validateStatusCode = (actualStatusCode: string): void => {
  const acceptedStatusCode: string[] = core
    .getInput('accept')
    .split(',')
    .filter((x) => x !== '')
    .map((x) => x.trim());
  if (!acceptedStatusCode.includes(actualStatusCode)) {
    throw new Error(
      `The accepted status code is ${acceptedStatusCode} but got ${actualStatusCode}`
    );
  }
};
  ...
  if (str) {
    ...
    if (matched.length > 0) {
      ...
      matched.forEach((matchedItem, index) => {
        const replaceResult = matchedItem.subMatches[0]
          .replace(/[\\]/g, '\\\\')
          .replace(/[\/]/g, '\\/')
          .replace(/[\b]/g, '\\b')
          .replace(/[\f]/g, '\\f')
