...
const builder = {
  basicAuth: (): AxiosBasicCredentials => {
    ...
    if (authArr.length !== 2) {
      throw new Error(
        'basic-auth format is invalid. The valid format should be username:password.'
      );
    }
    return {
      username: authArr[0],
  ...
  proxy: (): AxiosProxyConfig => {
    ...
    if (core.getInput('proxy-auth')) {
      const proxyAuthArr: string[] = core
        .getInput('proxy-auth')
        .trim()
        .split(':');
      if (proxyAuthArr.length !== 2) {
        throw new Error(
          'proxy-auth format is invalid. The valid format should be username:password.'
        );
      }
      proxy.auth = {
